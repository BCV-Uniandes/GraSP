...
class Endovis_2018(SurgicalDataset):
    ...
    def __getitem__(self, idx):
        ...
        complete_name = 'seq_{}_frame{}.{}'.format(video_name[4:], str(sec).zfill(self.zero_fill), self.image_type)
                ...
                if box_labels['bbox'] != [0,0,0,0]:
                    ...
                    if self.cfg.FEATURES.ENABLE:
                        ...
                        try:
                            ...
                            rpn_features.append(np.zeros(self.features_length))
        ...
        if self.cfg.REGIONS.ENABLE and len(ori_boxes):
            ...
            if self.cfg.FEATURES.ENABLE:
                if len(rpn_features)<max_boxes:
                    c_rpn_features = np.concatenate((rpn_features,np.zeros((max_boxes-len(rpn_features), self.features_length))),axis=0)
            ...
            if self.cfg.FEATURES.ENABLE:
                extra_data["rpn_features"] = np.zeros((max_boxes, self.features_length))

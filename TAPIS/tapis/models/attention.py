...
def attention_pool(tensor, pool, thw_shape, has_cls_embed=True, multiple_cls=1, norm=None):
    if pool is None:
    ...
    if has_cls_embed:
        cls_tok, tensor = tensor[:, :, :multiple_cls, :], tensor[:, :, multiple_cls:, :]
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, thw_shape):
        ...
        q, q_shape = attention_pool(
            ...
            multiple_cls=self.multiple_cls,
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x, thw_shape=None):
        ...
        x_res, _ = attention_pool(
            x, self.pool_skip, thw_shape, has_cls_embed=self.has_cls_embed, multiple_cls=self.multiple_cls,
        )

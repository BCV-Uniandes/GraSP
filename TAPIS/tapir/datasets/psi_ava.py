...
@DATASET_REGISTRY.register()
class Psi_ava_dense(SurgicalDataset):
    """
    PSI-AVA dataloader.
    """
    ...
    def __init__(self, cfg, split):
        ...
        self.fps = 1
        super().__init__(cfg,split)
    #     self.get_map_file(cfg)
    
    # def get_map_file(self, cfg):
    #     self.map_dict = json.load(open(os.path.join(cfg.ENDOVIS_DATASET.ANNOTATION_DIR,'map_file.json')))
    ...
    def __getitem__(self, idx):
        ...
        video_name = self._video_idx_to_name[video_idx]
        # center_idx = self.map_dict[video_name][sec]
        complete_name = '{}/{}.{}'.format(video_name, str(sec).zfill(self.zero_fill), self.image_type)

...
import tapir.datasets.surgical_dataset_helper as ava_helper
import tapir.evaluate.main_eval as psiava_eval
import tapir.utils.logging as logging
import tapir.utils.misc as misc
from tapir.utils.ava_eval_helper import (
    read_exclusions,
    read_labelmap,
)
...
IDENT_FUNCT_DICT = {'psi_ava': lambda x,y: 'CASE{:03d}/{:05d}.jpg'.format(x,y),
                    'psi_ava_dense': lambda x,y: 'CASE{:03d}/{:09d}.jpg'.format(x,y),
...
class SurgeryMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        self.features = cfg.FEATURES.VAL_FEATURES_PATH
        ...
        self.excluded_keys = read_exclusions(
            os.path.join(cfg.ENDOVIS_DATASET.ANNOTATION_DIR, cfg.ENDOVIS_DATASET.EXCLUSION_FILE) if cfg.ENDOVIS_DATASET.EXCLUSION_FILE is not ''
            else None
        )
        self.groundtruth = cfg.ENDOVIS_DATASET.COCO_ANN_DIR
    ...
    def finalize_metrics(self, epoch, log=True):
        ...
        for task,metric in zip(self.tasks, self.metrics):
            ...
            self.full_map[task] = psiava_eval.main_per_task(self.groundtruth, out_name[task], task, metric)
            if log:
                stats = {"mode": self.mode, "task":task, "map": self.full_map[task]}
                logging.log_json_stats(stats)
        if log:
            stats = {"mode": self.mode, "mean map": np.mean([v[m] for v,m in zip(list(self.full_map.values()), self.metrics)])}
            logging.log_json_stats(stats)
    ...
    def get_best_box(self, base_box, box_options):
        
        max_iou = 0
        best_box = None
        for box in box_options:
            iou = box_iou(box,base_box)
            if iou>max_iou:
                max_iou = iou
                best_box = box
        assert max_iou>0.9 and max_iou<=1
        return box_options[best_box]
    ...
    def save_json(self, task, preds, boxes, names, epoch):
        ...
        for idx, (pred, name) in enumerate(zip(preds[task], names)):
            assert name not in save_json_dict, f'Name {name} is repeated'
            task_key_name = f'{task}_logits'
            if self.regions and task in self._region_tasks:
                if self.segmentation:
                    try:
                        save_json_dict[name] = {'instances': [{"bbox":box, task_key_name:pred[b_id], "segment": self.get_best_box(box,self.region_proposals[name])} for b_id,box in enumerate(boxes[idx]) if box != [0,0,0,0]]}
                    except:
                        traceback.print_exc()
                        breakpoint()
                else:

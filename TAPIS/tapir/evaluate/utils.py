...
def format_instances(instances, width, height, segmentation=False):
    ...
    for instance in instances:  
        ...
        # pred_tools = instance['distr'][1:]
        pred_tools = instance['score_dist']
        this_instance = {'bbox': [x1,y1,x2,y2], 'tools_logits': pred_tools}
        if segmentation:
...
def read_detectron2_output(coco_anns_path, preds_path, selection, selection_params, segmentation=False):
    with open(coco_anns_path, 'r') as f:
    ...
    #     if len(data_dict[idx]['instances'])>0:
    #         key = data_dict[idx]['file_name']
    #         final_dict[key]={'bboxes':data_dict[idx]['instances']}
